...
class _MapScreenState extends State<MapScreen> {
    ...
    return Scaffold(
      ...
      body: GoogleMap(
        ...
        initialCameraPosition: CameraPosition(
          target: LatLng(6.9350, 79.9827), // Colombo coordinates
          zoom: 13,
        ),
        circles: Set.from([
          Circle(
            circleId: CircleId('Area1'),
            center: LatLng(6.9350, 79.9827), // Colombo coordinates for first area
  ...
  Set<Marker> _buildMarkers() {
    ...
    if (userLocations!.isNotEmpty) {
      markers.add(Marker(
        ...
        position: LatLng(
            userLocations!.last.latitude, userLocations!.last.longitude),
        infoWindow:
  ...
  Future<void> _calculateAndDrawRoute(LatLng origin, LatLng destination) async {
    ...
    String url = 'https://api.openrouteservice.org/v2/directions/driving-car?api_key=$apiKey&start=${origin.longitude},${origin.latitude}&end=${destination.longitude},${destination.latitude}';
    ...
    if (response.statusCode == 200) {
      ...
      if (features.isNotEmpty) {
        ...
        List<LatLng> points = geometry.map((point) => LatLng(point[1], point[0])).toList();
